...
import { formatError, extractCredentials } from '../utils/validation.js';
const validateGetUserBoards = (args) => {
    const schema = z.object({
        filter: z.enum(['all', 'open', 'closed']).optional()
    });
    return schema.parse(args);
};
const validateGetMember = (args) => {
    const schema = z.object({
...
export const trelloGetUserBoardsTool = {
...
export async function handleTrelloGetUserBoards(args) {
    try {
...
export const trelloGetMemberTool = {
...
export async function handleTrelloGetMember(args) {
    try {
        const { credentials, params } = extractCredentials(args);
