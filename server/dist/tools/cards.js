...
export async function handleCreateCard(args) {
    try {
...
export async function handleUpdateCard(args) {
    try {
        const { credentials, params } = extractCredentials(args);
...
export async function handleMoveCard(args) {
    try {
        const { credentials, params } = extractCredentials(args);
...
export async function handleGetCard(args) {
    try {
        const { credentials, params } = extractCredentials(args);
        ...
        if (downloadImages && attachments.length > 0) {
            const eligible = attachments.filter(a => a.mimeType &&
                IMAGE_MIME_TYPES.has(a.mimeType) &&
                a.bytes <= MAX_IMAGE_BYTES &&
                isAllowedAttachmentUrl(a.url));
            const skipped = attachments.filter(a => a.mimeType && IMAGE_MIME_TYPES.has(a.mimeType) && !eligible.includes(a));
...
const validateArchiveCard = (args) => {
...
export const trelloArchiveCardTool = {
...
export async function handleArchiveCard(args) {
    try {
        const { credentials, params } = extractCredentials(args);
